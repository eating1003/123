...
<%
    ...
    SqlConnection conn = new SqlConnection("server=.\\SQLEXPRESS;AttachDbFilename=D:\\sqldata\\company.mdf;;Integrated Security=True;Connect Timeout=30;User Instance=True");
    conn.Open();
    string strsql;
    strsql = "insert into shop(s_prod_id,s_prod_num,s_session,s_date) values('" + Request["proid"] + "','" + Request["prodnum"] + "','" + Session["car"] + "','" + date1.ToString("yyyy/MM/dd HH:mm") +"' ) ";
						...
						<li><a href="order.aspx" class="hvr-sweep-to-bottom">開始點餐</a></li>
							<li class="active"> <a href="#" class="dropdown-toggle hvr-sweep-to-bottom" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">餐點瀏覽<span class="caret"></span></a>
								<ul class="dropdown-menu">
                                  ...
                                  <%
         string cs = "server=.\\SQLEXPRESS;AttachDbFilename=D:\\sqldata\\company.mdf;;Integrated Security=True;Connect Timeout=30;User Instance=True" ;
...
<form id="form1" name="form1" method="post" action="car.aspx">
  <table width="80%" border="1" align="center">
    <tr>
      ...
      <td width="36%">金額</td>
          ...
          <%
              int price=0;
         string cs2 = "server=.\\SQLEXPRESS;AttachDbFilename=D:\\sqldata\\company.mdf;;Integrated Security=True;Connect Timeout=30;User Instance=True" ;
    ...
    <td><%Response.Write(dr2["s_prod_num"].ToString());   %> </td>
     <td><%Response.Write(dr2["價格"].ToString());
          // price +=  dr2["s_prod_num"].ToString   * dr2["價格"].ToString();
           price = price + (int.Parse(dr2["價格"].ToString()) * int.Parse(dr2["s_prod_num"].ToString()));
                         %> </td>    
 </tr>
           <%           
                    }
  ...
  <center> 
  總金額:<%//Response.Write(price);%><hr />
  <input type="submit" name="Submit" value="我要結帳" /> </center>
</form>
